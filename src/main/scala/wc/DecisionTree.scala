...
object DecisionTree {
  ...
  def main(args: Array[String]) {
    ...
    val ss = SparkSession.builder().appName("Decision Tree").master("local[1]").getOrCreate()
...
// Adding a vector feature column into the DataFrame,with all the relevant columns
...
//val predictionAndLabels = predictions.map(case Row(predictedLabel,indexedLabel) => (predictedLabel,indexedLabel)).rdd
//val predictionAndLabels: RDD[(Double,Double)] = predictions.select("predictedLabel","indexedLabel").rdd.map(row => (row(1).toDouble,row(2).toDouble)).mapValues()
//predictionAndLabels.take(5).foreach(println)
//.map{case Row(x,y) => (x,y)}.rdd
